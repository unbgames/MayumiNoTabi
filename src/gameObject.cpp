...
void PlayerControlFunc(GameObject* go, float time) {
	...
	if     (arrowReady && INPUT.key_is_down(KEY(a)) && curAnim == "idle")ac->ChangeCur("fire", false);
	else if (arrowReady && INPUT.key_is_down(KEY(s)) && curAnim == "idle")ac->ChangeCur("fire2",false);
	//else if (              INPUT.key_pressed (KEY(s)) && curAnim == "idle")ac->ChangeCur("kick", false);
	else{
